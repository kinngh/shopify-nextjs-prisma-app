...
type IfAllOptionalKeys<Obj, If, Else = never> = Obj extends Record<string, never> ? If : NonOptionalKeys<Obj> extends {
    length: 0;
} ? If : Else;
...
type AllowedRemoteChildren<Children, Root extends RemoteRoot<any, any>> = Children extends RemoteComponentType<string, any, any> ? RemoteComponent<Children, Root> : never;
type ExtractChildren<Type> = Type extends RemoteComponentType<string, any, infer Children> ? Children : never;
type AllowedChildren<Children extends RemoteComponentType<string, any> | boolean, Root extends RemoteRoot<any, any>, AllowString extends boolean = false> = Children extends true ? RemoteComponent<any, Root> | AllowedTextChildren<Root, AllowString> : Children extends false ? never : AllowedRemoteChildren<Children, Root> | AllowedTextChildren<Root, AllowString>;
type AllowedTextChildren<Root extends RemoteRoot<any, any>, AllowString extends boolean = false> = AllowString extends true ? RemoteText<Root> | string : RemoteText<Root>;
...
export type RemoteChild<Root extends RemoteRoot<any, any>> = RemoteComponent<any, Root> | RemoteText<Root>;
export type RemoteComponentSerialization<Type extends RemoteComponentType<string, any> = RemoteComponentType<string, any>> = {
...
export type Serialized<T> = T extends RemoteComponent<infer Type, any> ? RemoteComponentSerialization<Type> : T extends RemoteText<any> ? RemoteTextSerialization : never;
export {};
